...
const videoModels = [
  {value:'kling-v3-turbo',label:'Kling 3.0 Turbo',sound:true,camera:false,cfg:false,t2v:true,modes:['std','pro'],pricing:{std:1,pro:2},motion:false},
  {value:'kling-v3',label:'Kling 3.0',sound:true,camera:false,cfg:false,t2v:true,modes:['std','pro','4k'],pricing:{std:1,pro:2,'4k':6},motion:true,motionPricing:{std:1,pro:2},motionRequiresImage:true},
]
...
<template>
  <div class="create-page page-enter">
    ...
    <div v-if="currentMode==='fast'" class="step-block glass-panel glow">
      ...
      <div class="block-action"><span class="count-hint mono">{{ fastUseAI ? `图片 ${imageCost} + 视频 ${videoCost} = ${imageCost+videoCost} 积分` : `视频 ${videoCost} 积分` }} · 余额 {{ auth.user?.credits||0 }}</span><el-button type="primary" size="large" @click="fastCreate" :loading="loading" :disabled="!story.title||!story.content"><span class="btn-icon">⚡</span> 一键生成短剧</el-button></div>
    </div>
